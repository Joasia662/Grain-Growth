...
namespace GameOfLife
{
    static class MooreNeighborhood
    {
        static public int returnNextStep(int[,] matrix, int cellInRow, int cellInColumn, int yCellIndex, int xCellIndex, string condition)
        {
            ...
            logic.addToMap(ref neighbours, matrix[upperIndex, leftIndex]);
            logic.addToMap(ref neighbours, matrix[upperIndex, rightIndex]);
            ...
            logic.addToMap(ref neighbours, matrix[lowerIndex, leftIndex]);
            logic.addToMap(ref neighbours, matrix[lowerIndex, rightIndex]);
